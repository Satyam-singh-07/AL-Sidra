...
@section('content')
                        ...
                        <div class="row g-3 align-items-end">
                            ...
                            <div class="col-md-3">
                                ...
                                <select name="category" class="form-select" id="categoryFilter">
                                    ...
                                    <option value="basic-beliefs"
                                        {{ request('category') == 'basic-beliefs' ? 'selected' : '' }}>Basic Beliefs
                                    </option>
                                    <option value="prayer" {{ request('category') == 'prayer' ? 'selected' : '' }}>Prayer
                                        (Salah)</option>
                                    <option value="fasting" {{ request('category') == 'fasting' ? 'selected' : '' }}>Fasting
                                        (Sawm)</option>
                                    <option value="charity" {{ request('category') == 'charity' ? 'selected' : '' }}>Charity
                                        (Zakat)</option>
                                    <option value="pilgrimage" {{ request('category') == 'pilgrimage' ? 'selected' : '' }}>
                                        Pilgrimage (Hajj)</option>
                                    <option value="quran" {{ request('category') == 'quran' ? 'selected' : '' }}>Quranic
                                        Studies</option>
                                    <option value="hadith" {{ request('category') == 'hadith' ? 'selected' : '' }}>Hadith
                                        Studies</option>
                                    <option value="history" {{ request('category') == 'history' ? 'selected' : '' }}>Islamic
                                        History</option>
                                    <option value="ethics" {{ request('category') == 'ethics' ? 'selected' : '' }}>Islamic
                                        Ethics</option>
                                </select>
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="input-group flex-nowrap">
                                    ...
                                    <a href="{{ route('religion-info.index') }}" class="btn btn-outline-secondary">
                ...
                <div class="card info-card shadow-sm h-100">
                    <div class="card-body position-relative">
                        ...
                        <div class="info-actions mt-3">
                            <button class="btn btn-sm btn-outline-primary btn-view" data-id="{{ $info->id }}">
                                View
                            </button>

                            <button class="btn btn-sm btn-outline-warning btn-edit" data-id="{{ $info->id }}">
                                Edit
                            </button>

                            <form action="{{ route('religion-info.destroy', $info->id) }}" method="POST" class="d-inline"
                ...
                <div class="modal-body">
                    <form method="POST" action="{{ route('religion-info.store') }}" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="form-control editor-area" contenteditable="true"
                                placeholder="Enter detailed information here..."></div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    {{-- <button type="button" class="btn btn-primary" data-bs-toggle="modal"
                        data-bs-target="#editInfoModal" data-bs-dismiss="modal">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form id="editForm" method="POST">
                        ...
                        <div class="mb-3">
                            <label class="form-label">Content *</label>
                            <div class="form-control editor-area" id="editEditor" contenteditable="true"></div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const addForm = document.querySelector('#addInfoModal form');

            addForm.addEventListener('submit', function() {
                const editor = addForm.querySelector('.editor-area');
